...
export default function DailyClosingView({ dayStr }) {
  ...
  const data = useMemo(() => {
    ...
    const expenseTxns = transactions.filter((tx) => tx.type === 'expense' && toDayStr(tx.date) === dayStr)
  ...
  return (
    <div>
      ...
      <div id="printable-report" className="mx-auto max-w-2xl">
        ...
        <Section title={`💰 ${t('dailyClosing.revenue')}`} tone="border-gold/30 bg-gold/[0.06] text-gold hover:bg-gold/[0.1]" open={openSec.revenue} onToggle={() => toggle('revenue')}>
        ...
        {data.openReceivables.length > 0 && (
          <Section title={`📋 ${t('dailyClosing.receivables')}`} tone="border-sky-500/30 bg-sky-500/[0.06] text-sky-300 hover:bg-sky-500/[0.1]" open={openSec.receivables} onToggle={() => toggle('receivables')}>
            <div className="card p-5">
        ...
        <Section title={`🔴 ${t('dailyClosing.expenses')}`} tone="border-rose-500/30 bg-rose-500/[0.06] text-rose-300 hover:bg-rose-500/[0.1]" open={openSec.expenses} onToggle={() => toggle('expenses')}>
          <div className="card p-5">
        ...
        <Section title={`✅ ${t('dailyClosing.settlement')}`} tone="border-emerald-500/30 bg-emerald-500/[0.06] text-emerald-300 hover:bg-emerald-500/[0.1]" open={openSec.settlement} onToggle={() => toggle('settlement')}>
          <div className="card p-5">
