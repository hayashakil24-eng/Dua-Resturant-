...
**Status: ✅ complete, manual QA done.** The backend is complete (every `AppContext.jsx` mutator has a permission-gated, audited REST route) **and the frontend is wired to it** — `AppContext.jsx` now hydrates from the backend and every mutator hits a route; `Login.jsx` does real username/password auth (now with a self-signup + admin-approval path added on top, see [`07-post-phase1-features.md`](07-post-phase1-features.md)). A full manual QA pass drove every domain through the real running app (see "Manual QA findings" below) and found two real bugs, both now fixed. The `attendance` slice stays frontend-local for now (no real machine-attendance source exists yet); `overrideAttendance` already persists to the backend. See "Progress" at the bottom.

Current count, counted directly off every route file: **95 endpoints across 16 route files** (this doc previously said 86; the difference is endpoints added after Phase 1's original close-out, e.g. self-signup/approval and recipe edit/delete — see [`07-post-phase1-features.md`](07-post-phase1-features.md)). See [`api-reference.md`](api-reference.md) for the full current table. Treat that doc as the source of truth for the route list going forward; this doc's own endpoint list below is the *original Phase 1 scope*, not a live count.
...
- Tests: `test/domains.api.test.ts` covers each domain's happy path, a permission denial, and the settings→order GST-lock. Total `npm test` → 56 green at the time.

**Current count** (re-run for this docs update): `npm test` → **10 files, 69 tests, all passing**. The two API-test files (`orders.api.test.ts`, `domains.api.test.ts`) each reseed the full real menu in `beforeAll` (~13s), so a full `npm test` run takes ~1–2 minutes — if it looks hung under a short timeout, it isn't; let it run.
