...
**Cafe Ali — Restaurant/Hotel Management System.** This is a monorepo: `frontend/` holds the React + Vite + Tailwind CSS app, packaged as an **Electron desktop app** (per `requirements.md` §11 — offline-first desktop, not web-based); `backend/` is a TypeScript/Fastify/Prisma server that the frontend talks to over REST + Socket.IO (see "Backend" below — Phases 0–4 built, Phase 5 not started); `control-panel/` is a separate Electron app offering an alternative, simpler on-site install of the same backend (see `control-panel/README.md`). The data-layer swap described below is **built**: `AppContext.jsx` hydrates from the backend on mount (`fetch()` per collection) and every mutator posts to a permission-gated route, then refetches — it no longer uses `localStorage` for domain data (only the JWT is stored there, via `src/api/client.js`). Login is real (username/password → JWT). Live updates from other devices arrive over Socket.IO (Phase 2) — a change on one cashier's screen shows up on another's within about a second, no manual refresh. `mockData.js` still exists but is now only a seed reference + a couple of frontend-local fallbacks (e.g. `INITIAL_ATTENDANCE`, which stays local because machine-attendance isn't backend-fed yet); the Electron shell is untouched. Run the backend (`cd backend && npm run dev`) alongside the frontend, or the app shows a "Cannot reach the server" state. See `requirements.md` for the full client requirements doc (Roman Urdu original scope) and `requirements-conflicts.md` for open ambiguities that still need client sign-off before they're built.
...
- `main.js` also wires `electron-updater` (checks on launch + every 4h against `package.json`'s `build.publish`, a generic feed served by the VPS) and exposes `installUpdate`/`onUpdateAvailable`/`onUpdateDownloaded` via `preload.js` for `src/components/UpdateBanner.jsx` — see `docs/08-auto-update.md` and the "Auto-update" bullet under Backend below before touching either.
...
Status: **Phases 0–4 built and verified** (Phase 4 against a local SQLite stand-in for the VPS — no real Supabase/Postgres credentials exist yet). Phase 5 (cloud-facing features) is unstarted, candidate-list only. A batch of ordinary feature work also landed on top of the finished phases — staff self-signup/approval, recipe edit/delete, table shift, full business-day close — see `docs/07-post-phase1-features.md`, not covered by the phase docs below. `backend/` is a running Fastify + Prisma server: full schema (SQLite locally), ported business logic under `backend/src/core/`, JWT auth (`src/auth/`), a REST route per `AppContext.jsx` mutator (`src/routes/` → `src/services/`, permission-gated + audited — full current list in `docs/api-reference.md`), a Socket.IO layer broadcasting live updates to every connected device (`src/realtime/`), PM2-based process supervision + LAN discovery + scheduled local backups (`docs/04-phase-3-deployment-hardening.md`), an outbox-pattern sync job pushing to a central VPS instance (`src/sync/`, `src/vps/`), and a passing suite (`npm test` in `backend/` → 10 files / 69 tests, incl. HTTP smoke tests). **The frontend is wired to it** (see the data-layer section above). Run it with `cd backend && npm run dev` (listens on `:4000`; the frontend's `src/api/client.js` defaults there, overridable via `VITE_API_URL`). Demo logins: `admin`/`manager`/`cashier`/`kitchen`, password `1234`. Work continues phase-by-phase per `docs/` — see `docs/00-overview.md` for the index and each phase file's own status/verification notes for exactly what is and isn't proven yet (e.g. Phase 3's reboot-survival and Phase 4's real-Postgres behavior both still need the real hardware/credentials).

Two things worth knowing, because they shape how backend work should be framed:

- **Target architecture**: one shared TypeScript/Fastify/Prisma codebase deployed twice — a local server (SQLite) on a dedicated restaurant PC that every on-site device talks to over the LAN, and a central VPS server (PostgreSQL via Supabase, managed-hosting only) that the local server syncs to in the background via an outbox table (`OutboxEntry`, `src/sync/job.ts`). Day-to-day operations never depend on the VPS being reachable — the restaurant's internet connection is unreliable (beach location, weak WiFi), so this isn't an edge case, it's the normal operating condition to design for. This is now built, not just planned — see `docs/05-phase-4-vps-sync.md`.
- **The backend's REST routes mirror `AppContext.jsx`'s mutator functions 1:1** — same names, same permission checks (via a ported `permissions.js`), same audit-log shape. Every mutator is independently re-checked server-side (not just UI-gated) — this closed structurally in Phase 1, so it no longer needs chasing function-by-function in the frontend.
- **Alternative deployment**: `control-panel/` embeds this exact backend package (`@cafe-ali/backend`, via `backend/src/index.ts`'s public export surface) directly inside an Electron main process, instead of running it standalone under PM2. It's an alternative, not a replacement — see `control-panel/README.md` for the trade-off (loses PM2's OS-level crash supervision, gains a no-terminal install) and its device-management features (live connected-devices list, disconnect).
- **JWTs are no longer fully stateless**: login registers an in-memory session keyed by a `jti` (`src/auth/sessions.ts`), checked by both `authenticate()` and the Socket.IO handshake — this is what lets a session be force-revoked (`POST /api/auth/logout`, or the Control Panel's "disconnect device"). Because sessions are in-memory, **restarting the backend now requires every device to log in again** — expected, not a bug, and the correct trade-off for revocation to mean anything.
- **Auto-update is built** (`docs/08-auto-update.md`, per `requirements.md` §14): both Electron apps (`frontend/`, `control-panel/`) use `electron-updater` pointed at a `generic` publish feed served straight off the VPS — `src/vps/app.ts` registers `@fastify/static` over `env.updatesDir` (`UPDATES_DIR`, default `backend/updates/`) under `/updates/`, so shipping a new build is just building it and dropping the installer + `latest.yml` + `.blockmap` into that folder on the VPS, not a code change or redeploy. Each app checks on launch and every 4 hours, downloads quietly, and only surfaces anything once a new version is actually ready (a failed/offline check stays silent — unreliable internet is this app's normal state, not an error). `autoUpdater.installDirectory` is pinned to `path.dirname(process.execPath)` in both `main.js` files specifically because the combined installer (`installer/installer.nsi`) installs to a fixed `Program Files` path electron-builder's own default installer wouldn't otherwise know about — don't remove that line when touching update logic, or a silent update would reinstall to the wrong location. **The release workflow, once a client machine has the app installed once**: edit code → bump `version` in the changed app's `package.json` (`npm version patch`) → `npm run dist` in that app's directory → upload the 3 resulting files (the new installer `.exe` + its `.blockmap` first, `release/latest.yml` last) to `/opt/cafeali/app/backend/updates/<frontend|control-panel>/` on the VPS, replacing what's there (`docs/deployment-setup.md`'s "Publishing an app update" section has the exact commands). Nothing else is redeployed and no client machine is ever touched again — already-installed apps pick up the new version on their own next check.

An MCP server was raised during planning as a possible future direction, **not a requirement or a scheduled phase** — see `docs/99-future-considerations.md`. Don't build toward it speculatively; it only influenced a structural preference (keep business logic in one core service layer, not inside route handlers) that Phase 0 already covers for its own reasons.
