...
**Cafe Ali — Restaurant/Hotel Management System.** This is a monorepo-in-progress: `frontend/` holds the React + Vite + Tailwind CSS app, packaged as an **Electron desktop app** (per `requirements.md` §11 — offline-first desktop, not web-based); `backend/` has a real TypeScript/Fastify/Prisma workspace as of Phase 0, but **no HTTP server or routes yet and nothing wired to the frontend** (see "Backend" below — Phase 0 complete, Phase 1 not started). The frontend still runs entirely on mock data — no live backend, no API calls. All "persistence" is `localStorage`; all "database" is `frontend/src/data/mockData.js`. When the backend is wired in (Phase 1), that's purely a data-layer swap in `AppContext.jsx` (localStorage → `fetch()` calls) — it doesn't touch the Electron shell. See `requirements.md` for the full client requirements doc (Roman Urdu original scope) and `requirements-conflicts.md` for open ambiguities that still need client sign-off before they're built.
...
Status: **Phase 0 (foundation) complete; Phase 1 not started.** `backend/` now has a real TypeScript workspace (Fastify + Prisma), a full schema (SQLite locally), ported business logic under `backend/src/core/` (`permissions.ts`, `inventoryFlow.ts`, `orderTotal.ts`, `cost.ts`, `payroll.ts`, `accounting.ts`, `closing.ts`, `ids.ts`), a seed script, and a passing test suite (`npm test` in `backend/`). **There is still no HTTP server, no auth, and the frontend is not wired to it** — `frontend/` is unchanged and still runs entirely on mock data / `localStorage`, exactly as described above. Work proceeds phase-by-phase per `backend/docs/` — see `backend/docs/00-overview.md` for the index and each numbered phase file for its own status.
