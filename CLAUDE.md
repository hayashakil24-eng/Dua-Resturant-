...
Status: **Phases 0–4 built and verified** (Phase 4 against a local SQLite stand-in for the VPS — no real Supabase/Postgres credentials exist yet). Phase 5 (cloud-facing features) is unstarted, candidate-list only. `backend/` is a running Fastify + Prisma server: full schema (SQLite locally), ported business logic under `backend/src/core/`, JWT auth (`src/auth/`), a REST route per `AppContext.jsx` mutator (`src/routes/` → `src/services/`, permission-gated + audited), a Socket.IO layer broadcasting live updates to every connected device (`src/realtime/`), PM2-based process supervision + LAN discovery + scheduled local backups (`docs/04-phase-3-deployment-hardening.md`), an outbox-pattern sync job pushing to a central VPS instance (`src/sync/`, `src/vps/`), and a passing suite (`npm test` in `backend/` → 56, incl. HTTP smoke tests). **The frontend is wired to it** (see the data-layer section above). Run it with `cd backend && npm run dev` (listens on `:4000`; the frontend's `src/api/client.js` defaults there, overridable via `VITE_API_URL`). Demo logins: `admin`/`manager`/`cashier`/`kitchen`, password `1234`. Work continues phase-by-phase per `backend/docs/` — see `backend/docs/00-overview.md` for the index and each phase file's own status/verification notes for exactly what is and isn't proven yet (e.g. Phase 3's reboot-survival and Phase 4's real-Postgres behavior both still need the real hardware/credentials).

Two things worth knowing, because they shape how backend work should be framed:

- **Target architecture**: one shared TypeScript/Fastify/Prisma codebase deployed twice — a local server (SQLite) on a dedicated restaurant PC that every on-site device talks to over the LAN, and a central VPS server (PostgreSQL via Supabase, managed-hosting only) that the local server syncs to in the background via an outbox table (`OutboxEntry`, `src/sync/job.ts`). Day-to-day operations never depend on the VPS being reachable — the restaurant's internet connection is unreliable (beach location, weak WiFi), so this isn't an edge case, it's the normal operating condition to design for. This is now built, not just planned — see `backend/docs/05-phase-4-vps-sync.md`.
