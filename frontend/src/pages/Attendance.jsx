...
export default function Attendance() {
  ...
  return (
    <div>
      <PageHeader title={t('attendance.title')} subtitle={dateLong()} />

      <div className="mb-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <StatCard icon={IconUsers} label={t('attendance.onDuty')} value={counts.present} sub={t('attendance.presentLate')} />
        <StatCard icon={IconAlert} label={t('attendance.lateToday')} value={counts.late} sub={t('attendance.afterGrace')} />
        <StatCard icon={IconClock} label={t('attendance.checkedOut')} value={counts.done} sub={t('attendance.shiftCompleted')} />
        <StatCard icon={IconUsers} label={t('attendance.absent')} value={counts.absent} sub={t('attendance.notCheckedIn')} />
      </div>
          ...
          <table className="w-full min-w-[720px] text-left text-sm">
            <thead>
              <tr className="border-b border-ink-line text-xs uppercase tracking-wider text-cream-dim">
                <th className="px-5 py-4 font-semibold">{t('attendance.colStaff')}</th>
                <th className="px-5 py-4 font-semibold">{t('attendance.colRole')}</th>
                <th className="px-5 py-4 font-semibold">{t('attendance.colShift')}</th>
                <th className="px-5 py-4 font-semibold">{t('attendance.colCheckIn')}</th>
                <th className="px-5 py-4 font-semibold">{t('attendance.colCheckOut')}</th>
                <th className="px-5 py-4 font-semibold">{t('attendance.colStatus')}</th>
                <th className="px-5 py-4 font-semibold">{t('attendance.colSource')}</th>
                {canOverride && <th className="px-5 py-4 text-right font-semibold">{t('attendance.colAction')}</th>}
              </tr>
            </thead>
              ...
              {roster.map((s) => {
                ...
                return (
                  <tr key={s.id} className="transition hover:bg-white/[0.02]">
                    ...
                    <td className="px-5 py-4 text-cream-dim">{t(`roles.${s.role}`, s.role)}</td>
                    <td className="px-5 py-4 text-cream-dim">{t(`shifts.${s.shift}`, s.shift)}</td>
                    ...
                    {canOverride && (
                      <td className="px-5 py-4 text-right">
                        ...
                        >
                          <IconEdit size={14} /> {t('attendance.override')}
                        </button>
                      </td>
                    )}
                  </tr>
      ...
      {canOverride && manualEntries.length > 0 && (
        <div className="card mt-6 p-6">
          <div className="mb-4 flex items-center gap-2">
            ...
            <h2 className="font-serif text-xl text-cream">{t('attendance.manualHistory')}</h2>
          </div>
          <div className="space-y-3">
            {manualEntries.map(({ staff: s, record }) => (
              <div
                key={s.id}
                className="flex flex-col gap-2 rounded-2xl border-s-2 border-amber-500/50 bg-ink-soft p-4 sm:flex-row sm:items-start sm:justify-between"
              >
                <div>
                  <p className="font-medium text-cream">{s.name}</p>
                  ...
                  <p className="mt-1 text-xs text-cream-dim">
                    {t('attendance.in')} {time(record.checkIn)} · {t('attendance.out')} {time(record.checkOut)}
                  </p>
                </div>
                <div className="text-xs text-cream-dim sm:text-right">
                  <p>{t('attendance.by')}: {record.manualEntry.enteredBy}</p>
