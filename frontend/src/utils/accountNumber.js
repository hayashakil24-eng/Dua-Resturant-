...
// Kept textually parallel with backend/src/core/onlineAccount.ts — the server
// re-checks every save independently, so a rule change is a two-file edit.
...
export function accountFieldsFor(type) {
...
// the field can't hold a value that Save would only reject afterwards.
export function sanitizeAccountNumber(type, raw) {
...
export function sanitizeIban(raw) {
...
export function validateAccountNumber(type, value) {
